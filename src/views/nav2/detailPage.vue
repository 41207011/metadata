<template>
  <section>
          ...
          <el-tabs type="border-card" style="height:600px;" v-model="activeName">
              ...
              <el-tab-pane name="2" label="关系" >
                  <el-col :span="24" v-show="relationshipIf">
                      <!--关联信息模态框-->
                      <el-dialog :title="series.name" v-model="relationShipModal" :close-on-click-modal="true" top="0%" size="full" :modal-append-to-body="false"  style="float: left;position: absolute;width: 350px;height: 350px;">
                          <el-form>
      ...
      <!--编辑标签界面-->
      <el-dialog title="编辑标签" v-model="editTagFormVisible" :close-on-click-modal="false">
          <el-form :model="editTagForm" :rules="editTagFormRules" ref="editTagForm">
              ...
              <el-col class="box" :span="24" v-if="addTimeBtn_">
                  <el-col class="box2" :span="24" v-for="(timeOne,index) in editTagForm.validityPeriods" :key="index">
                      ...
                      <el-button icon="delete" type="danger" title="删除" @click="deleteTimeOne_(index)"
                                 style="margin-left: 20px; ;"></el-button>
                  </el-col>
              </el-col>
...
<script>
    ...
    import {getEntityDetails,typedefs,deleteTag,addTag,getTerm,deleteTerm,addTerm,getAuditDetails,lineage,editTag,spliceTerm} from  '../../api/api'
    export default {
        data() {
            return {
                activeName:'0',
        ...
        methods: {
            rJump(guid,name){
                if(guid!==this.$route.query.guid){
                    this.$router.push({ name: '数据详情', query: { guid: guid ,name:name}})
                }
            },
            tagChange(val){
            ...
            getEntityDetails_(){
                getEntityDetails(this.$route.query.guid).then(res=>{
                    this.entityMessage =res.data;
                    this.entityName = this.$route.query.name
                    this.typeName = this.entityMessage.entity.typeName
                    //表数据添加
                    let relationship = Object.assign({},res.data.entity.relationshipAttributes);
                    let data = [];
                    let link = [];
                    data.push({name:this.typeName,draggable:true, itemStyle: {normal: {color: '#1984c7'}},symbol:'image://static/image/file4.png'})
                    for(let key in relationship){
                        if(relationship[key]&&relationship[key].length!==0){
                            data.push({name:key,draggable:true,symbol:'image://static/image/file3.png'})
            ...
            getLineAge:function(){
             ...
             lineage(params).then(res=>{
                 //表数据添加
                 let lineAge = Object.assign({},res.data);
                 let data = [];
                 let link = [];
            ...
            showModal2(){
                ...
                this.chartBloodline.on('dblclick', function (params) {
                    this_.$router.push({ name: '数据详情', query: { guid: params.data.id ,name:params.name}})
                });
            },
        },
        mounted() {
