<template>
    <section>
        <el-row :gutter="20">
                ...
                <el-form>
                    <el-form-item style="margin-bottom: 0px;">
                        <div style="float: right;margin-top: 2px;">
                            <el-button type="primary" size="small" title="创建字典" @click="creatGlossaryModal(0)"><i class="fa fa-plus"></i></el-button>
                                <el-button type="primary" size="small" title="刷新" ><i class="fa fa-refresh"></i></el-button>
                        </div>
                    </el-form-item>
                </el-form>
                <div v-show="typeSwitch">
                    <el-input placeholder="搜索字典,分类" v-model="searchWord2"
            ...
            <el-col :span="19" v-show="standardNameId">
                ...
                <div >
                    <el-col :span="24" class="toolbar">
                        ...
                        <span  class=" pageShow" style="float: right;">如果您在下面的搜索结果中找不到数据字典明细，那么您可以<a @click="createEntityModel">创建新的数据字典明细</a></span>
                    ...
                    <el-table :data="allUser" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
                        ...
                        <el-table-column prop="name" label="字典名"  :show-overflow-tooltip="true">
                        ...
                        <el-table-column prop="fatherCode" label="父编码" :show-overflow-tooltip="true">
                        </el-table-column>
                        <el-table-column prop="createDate" label="创建时间" :show-overflow-tooltip="true">
                        </el-table-column>
                        <el-table-column prop="updateDate" label="更新时间" :show-overflow-tooltip="true">
                        </el-table-column>
                    <!--    <el-table-column prop="statusFlag" label="状态"  sortable :show-overflow-tooltip="true">
                        </el-table-column>-->
            ...
            <!--创建术语表-->
            <el-dialog title="创建字典" v-model="createGlossaryFormVisible" :close-on-click-modal="false">
                <el-form  label-width="70px"  :model="createGlossaryForm" :rules="createGlossaryFormRules" ref="createGlossaryForm">
                    ...
                    <el-form-item label="类型" prop="type">
                        <el-input v-model="createGlossaryForm.type"></el-input>
                    </el-form-item>
                    <el-form-item label="备注">
                        <el-input v-model="createGlossaryForm.remark"></el-input>
                    </el-form-item>
            ...
            <!--创建类别-->
            <el-dialog title="修改字典" v-model="createCagFormVisible" :close-on-click-modal="false">
                <el-form  label-width="70px"  :model="createCagForm" :rules="createCagFormRules" ref="createCagForm">
                    ...
                    <el-form-item label="类型" prop="type">
                        <el-input v-model="createCagForm.type"></el-input>
                    </el-form-item>
                    <el-form-item label="备注">
                        <el-input v-model="createCagForm.remark"></el-input>
                    </el-form-item>
            ...
            <!--创建实体-->
            <!--创建实体-->
            <el-dialog title="创建数据字典明细" v-model="createEntityFormVisible" :close-on-click-modal="false">
                <el-form    :model="createEntityForm" :rules="createEntityFormRules" ref="createEntityForm">

                    <el-form-item label="字典名" prop="name" label-width="120px" :rules="createEntityFormRules.name">
                        <el-input v-model="createEntityForm.name"></el-input>
                    </el-form-item>
                    <!-- <el-form-item label="状态" label-width="120px"  prop="statusFlag" :rules="createEntityFormRules.name">
                         <el-input v-model="createEntityForm.statusFlag"></el-input>
                     </el-form-item>-->
                    <el-form-item label="编码" label-width="120px"  prop="code" >
                        <el-input v-model="createEntityForm.code"></el-input>
                    </el-form-item>
                    <el-form-item label="父编码" label-width="120px"  prop="fatherCode">
                        <el-input v-model="createEntityForm.fatherCode"></el-input>
                    </el-form-item>
                    <el-form-item label="标记"label-width="120px"  prop="remark">
                        <el-input v-model="createEntityForm.remark"></el-input>
                    </el-form-item>
...
<script>
    import {DeleteDateDictionaryDetails,AddOrUpdateDateDictionaryDetails,selectDateDictionaryDetails,DeleteDateDictionaryType,AddOrUpdateDateDictionaryType,SelectDateDictionaryType,OrUpdate,saveOrUpdate,remove,getSTable,createEntity,deleteCategory,getCategory,typedefs,seachUsingBasic,deleteTag,deleteTerm,addTag,getTerm,spliceTerm2,addTerm,addAttributes,createTag,DeleteTag,getTermDetails,putTerm,getRelation,putCategory,putGlossary,createGlossary,deleteGlossary,postTerm,deleteTerm_,creatCategory,demo} from '../../api/api'
    export default {
        data() {
            return {
                standardNameId:'',
                ...
                createGlossaryFormRules:{
                    ...
                    remark:{required:true,message:'请输入字典备注'}
                },
                //创建术语表
                //创建实体
        ...
        methods: {
            ...
            getData3(){
                ...
                SelectDateDictionaryType().then(res=>{
                ...
                this.$refs['createGlossaryForm'].validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            AddOrUpdateDateDictionaryType(params).then((res) => {
                                this.addLoading = false;
                                this.$message({
                                    message: '创建字典成功',
                                    type: 'success'
            ...
            createCag_: function () {
                ...
                this.$refs['createCagForm'].validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            this.addLoading = true;
                            let params = Object.assign({id:this.upId,parentId:this.upParentId},this.createCagForm)
                            AddOrUpdateDateDictionaryType(params).then((res) => {
                                this.addLoading = false;
                                //NProgress.done();
                                this.$message({
                ...
                this.$refs['createEntityForm'].validate((valid) => {
                    if (valid) {
                        this.$confirm('确认提交吗？', '提示', {}).then(() => {
                            ...
                            let params
                            if(this.standardId){
                                params = Object.assign({id:this.standardId,foreignKey:this.standardNameId},this.createEntityForm)
                            }
                            else{
                                params = Object.assign({foreignKey:this.standardNameId},this.createEntityForm)
                            }
                            AddOrUpdateDateDictionaryDetails(params).then((res) => {
                                this.addLoading = false;
                                //NProgress.done();
                                ...
                                let params2 = {foreignKey: this.standardNameId,offset:(this.page-1)*this.startPageCount,limit:this.startPageCount}
                                selectDateDictionaryDetails(params2).then(res=>{
            ...
            creatGlossaryModal: function (val) {
                ...
                this.createGlossaryForm = {name:'',type:'',remark:''}
            ...
            //创建类别显示
            createCag(id,parentId,text,type,remark){
                this.upId = id
                this.upParentId = parentId
                this.createCagFormVisible = true;
                this.createCagForm = {name:text,type:type,remark:remark}
            },
            //删除术语表
            ...
            deleteCag_:function (id) {
                this.$confirm('是否删除该字典?','提示',{
                    ...
                    cancelButtonText:'取消',
                    type:'warning'}).then(() => {
                    DeleteDateDictionaryType({id:id}).then(res=>{
                        this.$message({
                            type: 'success',
                            message: '删除成功!'
                        });
            ...
            renderContent2(h, { node, data, store }) {
                ...
                width="150"
                trigger="hover">
                    <div class='popoverDiv' style='font-size: 14px !important;color: #666 !important;cursor:pointer'  on-click={ ()=>this.creatGlossaryModal(data.id)}><i class="fa fa-plus fa-fw" style="margin-left:20px;margin-right:10px"></i>创建子字典</div>
                <div  class='popoverDiv' style='font-size: 14px !important;color: #666 !important;cursor:pointer'  on-click={ ()=>this.createCag(data.id,data.parentId,data.text,data.attributes.type,data.attributes.remark)}><i class="fa fa-edit fa-fw" style="margin-left:20px;margin-right:10px"></i>修改字典</div>
                <div class='popoverDiv' style='font-size: 14px !important;color: #666 !important;cursor:pointer' on-click={()=>this.deleteCag_(data.id)}><i class="fa fa-trash fa-fw" style="margin-left:20px;margin-right:10px"></i>删除字典</div>
                <i class='fa fa-ellipsis-h' slot="reference" ></i>
                    </el-popover>
            ...
            remove2:function (index) {
                this.$confirm('是否删除该数据字典明细?','提示',{
                    confirmButtonText:'确定',
                    cancelButtonText:'取消',
                    type:'warning'}).then(() => {
                    DeleteDateDictionaryDetails({id:this.allUser[index].id}).then(res=>{
                        let code = res.data.code
                        if(code===1){
                            this.$message({
                                message: '操作失败',
                            ...
                            let params = {foreignKey:this.standardNameId,offset:(this.page-1)*this.startPageCount,limit:this.startPageCount}
                            selectDateDictionaryDetails(params).then(res=>{
                                this.allUser = res.data.rows
                                this.total = res.data.total
                                this.startPage = (this.page-1)*this.startPageCount
                                this.endPage = (this.page)*this.startPageCount
