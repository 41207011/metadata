<template>
    <section>
        <el-row :gutter="20">
            ...
            <el-col :span="19">
                ...
                <div >
                    <el-col :span="24" class="toolbar">
                        ...
                        <span  class=" pageShow" style="float: right;">如果您在下面的搜索结果中找不到该标准表，那么您可以<a @click="createEntityModel">创建新的编码标准</a></span>
                    ...
                    <el-table :data="allUser" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
                        ...
                        <el-table-column prop="name" label="标准名"  :show-overflow-tooltip="true">
                            <!--   <template scope="scope">
                                   {{scope.row.name}}
                                   &lt;!&ndash;<router-link :to="{ name: '数据详情', query: { guid: scope.row.guid ,name:scope.row.name}}">{{scope.row.name}}</router-link>&ndash;&gt;
                               </template>-->
                        </el-table-column>
                        <el-table-column prop="rule" label="编码规则" :show-overflow-tooltip="true">
                        </el-table-column>
                        <el-table-column prop="code" label="编码" :show-overflow-tooltip="true">
                        </el-table-column>
                        <el-table-column prop="relatedtable" label="关联表" :show-overflow-tooltip="true">
                        </el-table-column>
                        <el-table-column prop="relatedcolum" label="关联列" :show-overflow-tooltip="true">
                        </el-table-column>
            ...
            <!--创建实体-->
            <el-dialog title="创建编码标准" v-model="createEntityFormVisible" :close-on-click-modal="false">
                <el-form    :model="createEntityForm" :rules="createEntityFormRules" ref="createEntityForm">

                    <el-form-item label="标准名" prop="name" label-width="120px" :rules="createEntityFormRules.name">
                        <el-input v-model="createEntityForm.name"></el-input>
                    </el-form-item>
                    <el-form-item label="编码规则" label-width="120px"  prop="rule">
                        <el-input v-model="createEntityForm.rule"></el-input>
                    </el-form-item>
                    <el-form-item label="关联表" label-width="120px"  prop="relatedtable" >
                        <el-input v-model="createEntityForm.relatedtable"></el-input>
                    </el-form-item>
                    <el-form-item label="关联列" label-width="120px"  prop="relatedcolum" >
                        <el-input v-model="createEntityForm.relatedcolum"></el-input>
                    </el-form-item>
        ...
        methods: {
            ...
            if (valid) {
                this.$confirm('确认提交吗？', '提示', {}).then(() => {
                    ...
                    codingRuleSaveOrUpdate(params).then((res) => {
                        this.addLoading = false;
                        //NProgress.done();
                        ...
                        if(code===1){
                            ...
                            codingRuleList(params2).then(res=>{
            ...
            remove2:function (index) {
                this.$confirm('是否删除该编码规则?','提示',{
                    confirmButtonText:'确定',
                    cancelButtonText:'取消',
                    type:'warning'}).then(() => {
                    codingRuleRemove({id:this.allUser[index].id}).then(res=>{
                        let code = res.data.code
                        if(code===1){
                            this.$message({
                                message: '操作失败',
            ...
            ggetUser(){
                ...
                codingRuleList(params).then(res=>{
                    this.allUser = res.data.rows
                    this.total = res.data.total
                    this.startPage = (this.page-1)*this.startPageCount
                    this.endPage = (this.page)*this.startPageCount
