<template>
    <section id="sss">
        <!--编辑-->
        <el-dialog title="编辑编码规则" v-model="zz1" :close-on-click-modal="false">
            <el-form  label-width="90px"  :model="zz1Form" ref="zz1Form">
                <el-form-item label="表名"  prop="name" :rules="zz1FormRules.req" >
                    <el-input v-model="zz1Form.name" ></el-input>
                </el-form-item>
                <el-form-item label="主键" prop="key" :rules="zz1FormRules.req">
                    <el-input v-model="zz1Form.key"></el-input>
                </el-form-item>
                <el-form-item label="规则" prop="rule" :rules="zz1FormRules.req">
                    <el-input v-model="zz1Form.rule"></el-input>
                </el-form-item>
                <el-form-item label="编码类型" prop="type" :rules="zz1FormRules.req">
                    <el-input v-model="zz1Form.type"></el-input>
                </el-form-item>
                <el-form-item label="状态" prop="status" :rules="zz1FormRules.req">
                    <el-select v-model="zz1Form.status">
                        <el-option value="完成">完成</el-option>
                        <el-option value="未编码">未编码</el-option>
                    </el-select>
                </el-form-item>
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click.native="zz1 = false">取消</el-button>
                <el-button type="primary" @click.native="zz1Submit" :loading="addLoading">提交</el-button>
            </div>
        </el-dialog>
        <!--添加-->
        <el-dialog title="添加编码规则" v-model="zz2" :close-on-click-modal="false">
            <el-form  label-width="90px"  :model="zz1Form" ref="zz2Form">
                <el-form-item label="表名"  prop="name" :rules="zz1FormRules.req" >
                    <el-input v-model="zz1Form.name" ></el-input>
                </el-form-item>
                <el-form-item label="主键" prop="key" :rules="zz1FormRules.req">
           ...
           <!--     <el-form-item label="状态" prop="status" :rules="zz1FormRules.req">
            ...
            <div slot="footer" class="dialog-footer">
                <el-button @click.native="zz2 = false">取消</el-button>
                <el-button type="primary" @click.native="zz2Submit" :loading="addLoading">提交</el-button>
            </div>
        </el-dialog>
        <el-row :gutter="20" style="padding: 15px">
            <el-col :span="5" id="leftDiv">
                <el-card class="box-card" style="min-height: 500px">
                    <div slot="header" class="clearfix">
                        <span style="line-height: 36px;">编码主题</span>
                    ...
                    <div >
                        ...
                        <el-input placeholder="搜索编码主题" v-model="searchWord"
            ...
            <el-col :span="19">
                <el-card class="box-card">
                    <div slot="header" class="clearfix">
                        <span style="line-height: 36px;">编码规则</span>
                        <!--<el-button type="primary" size="small" title="创建术语表" @click="creatGlossaryModal()"><i class="fa fa-plus"></i>
                        </el-button>
                        <el-button type="primary" size="small" title="刷新" @click="refresh"><i class="fa fa-refresh"></i></el-button>-->
                    </div>
                    <div >
                          <el-table :data="table_" >
                              <el-table-column label="表名" prop="name"></el-table-column>
                              <el-table-column label="主键" prop="key"></el-table-column>
                              <el-table-column label="规则" prop="rule"></el-table-column>
                              <el-table-column label="编码类型" prop="type"></el-table-column>
                              <el-table-column label="状态" prop="status"></el-table-column>
                              <el-table-column label="操作">
                                  <template scope="scope">
                                      <el-button size="mini" type="primary" @click="zzStart(scope.$index)" icon="caret-left" title="启动"></el-button>
                                      <el-button size="mini" type="info" @click="zzEdit(scope.$index)" icon="edit" title="编辑"></el-button>
                                      <el-button size="mini" type="danger" @click="zzRemove(scope.$index)" icon="delete" title="删除"></el-button>
                                  </template>
                              </el-table-column>
                          </el-table>
                        ...
                        <el-row class="toolbar">
                            <el-button type="primary" @click="addRules">添加规则</el-button>
                                <el-pagination layout="sizes,prev, pager, next, jumper" @current-change="handleCurrentChange" @size-change="handleSizeChange" :page-size="startPageCount" :page-sizes="[10, 20, 50, 100]" style="float:right;" :total="table_.length" >
                                </el-pagination>
                        </el-row>
                    </div>
                </el-card>
             <!--   &lt;!&ndash;工具条&ndash;&gt;
               <div v-if="chooseTerm&&!typeSwitch">
                ...
                <el-table :data="allUser" highlight-current-row v-loading="listLoading" @selection-change="selsChange"
                  ...
                  &lt;!&ndash;  <el-table-column prop="term" label="术语" width="250" sortable>
                        <template scope="scope">
                ...
                <div v-if="typeSwitch===true">
                    <el-row :gutter="10" style="max-height: 756px;overflow: auto ">
                        <el-col :span="12"  v-for="(one,index) in demoData" style="margin-top: 10px;">
                            <el-card class="box-card draggle-div"  style="height: 270px; overflow: auto;">
                                <div slot="header" class="clearfix">
            ...
            <!--<el-col :span="19" style="margin-top: 20px">
                <el-card class="box-card">
                    <div slot="header" class="clearfix">
                        <span style="line-height: 36px;">编码任务</span>
                        &lt;!&ndash;<el-button type="primary" size="small" title="创建术语表" @click="creatGlossaryModal()"><i class="fa fa-plus"></i>
                        </el-button>
                        <el-button type="primary" size="small" title="刷新" @click="refresh"><i class="fa fa-refresh"></i></el-button>&ndash;&gt;
                    </div>
                    <div >
                        <el-table :data="table_" >
                            <el-table-column label="表名" prop="name"></el-table-column>
...
<script>
    import {deleteCategory,getCategory,typedefs,seachUsingBasic,deleteTag,deleteTerm,addTag,getTerm,addTerm,addAttributes,createTag,DeleteTag,getTermDetails,putTerm,getRelation,putCategory,putGlossary,createGlossary,deleteGlossary,postTerm,deleteTerm_,creatCategory,demo,spliceTerm} from '../../api/api'
    export default {
        data() {
            return {
                zz1FormRules:{req:{required:true,message:'该项不能为空'}},
                chooseEditIndex:0,
                zz1Form:{
                    name:'',
                    key:'',
            ...
            //创建类别
                ...
                searchWord: '',
                searchWord2:'',
                data1: [{label:'用户',children:[{label:'用户映射规则'}]},{label:'财务'},{label:"物料"}],
                defaultProps: {
                    children: 'children',
                    label: 'label'
                },
        ...
        methods: {
            zzStart(index){
                this.$confirm('是否启动该规则?','提示',{
                    confirmButtonText:'确定',
                    cancelButtonText:'取消',
                    type:'warning'}).then(() => {
                    this.table_[index].status = '完成'
                    this.$message({
                        type: 'success',
                        message: '启动成功!'
                    });
            ...
            zz2Submit(){
                this.$refs['zz2Form'].validate(valid=>{
                    if(valid){
                        this.$message({
                            type: 'success',
                            message: '添加成功!'
                        });
                        this.table_.push(this.zz1Form)
                        this.zz2=false
                    }
                    else{
                        return false
            ...
            zz1Submit(){
                this.$refs['zz1Form'].validate(valid=>{
                    if(valid){
                        this.$message({
                            type: 'success',
                            message: '编辑成功!'
                        });
                        this.table_[this.chooseEditIndex] = Object.assign({},this.zz1Form)
                        this.zz1=false
                    }
                    else{
                        return false
            ...
            zzEdit(index){
               this.zz1 = true
               this.zz1Form = Object.assign({},this.table_[index])
              ...
              if(data.categoryGuid){
                  ...
                  getCategory(data.categoryGuid).then(res=>{
                      ...
                      for(let value of this.glossary){
                          ...
                          if(value.name===qName.substring(qName.indexOf('@')+1,qName.length)){
